...
pipeline {
    ...
    stages {
        ...
        stage('Clone Source') {
            steps {
                git: 'https://github.com/EfxAnsh/my-app.git'
            }
        }
        ...
        stage('Login to Docker Hub') {
            steps {
                withCredentials([string(credentialsId: 'dockerhub-token', variable: 'DOCKER_TOKEN')]) {
                    sh 'echo $DOCKER_TOKEN | docker login -u anshasx --password-stdin'
                }
            }
        }
...
- Payload URL: Formatted as:  https://disarray-thrill-backdrop.ngrok-free.dev/github-webhook/(mine)
...
| **Stage 5** | **Push to Registry** | The image is authenticated and pushed to the `anshasx/my-app` Docker Hub repository. |
